...
- **Real language data.** If the course will need examples of local-language text, file
  contents or settings, ask the SME to supply the actual data and record it verbatim,
  character for character. You cannot read these languages and must not generate them —
  and an invented substitute would throw away the realism the course depends on. Note who
  supplies anything still missing so `module-author` can leave a placeholder rather than
  fill the gap.
...
Remember who the course is for: a consultant who does **not** speak the language being
translated, often working on the first translation ever into it. Objectives must therefore
describe diagnosing the tool, the data and the workflow — never judging whether the
translated text itself is correct, which is the translation team's call, not the
consultant's and not yours.
...
- **No inventing language data.** Never produce local-language text or glosses, or state
  facts about a language's script, tone marking or character set, and never treat data you
  cannot read as something you can interpret. It comes from the SME verbatim, or it stays
  a marked gap in the design doc.
