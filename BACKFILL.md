...
| `coretech-malware` | SIL Learning Hub: https://courses.sil.org/instructor_modules/list/1932669 | **In repo** (backfilled 2026-08-11 from Learning Hub source pasted/exported by Jenni): 4 modules + 2 per-module quizzes (QTI) + reflection + revision-tutor helper, all as markdown. **Known follow-ups (flagged in-file, not resolved):** Module 3 is a thin worksheet needing a 4Cs rewrite; Quiz B Q4 keys to "Malwarebytes" (not in the modules); frontmatter `With Assistance` conflicts with the modules' "Has Knowledge"; Module 3 DOCX + Module 1 scan video/screenshots not supplied. | |
| `coretech-os-basics` | Cypher for Business (LMS) | **Converted, and has since entered the pipeline as an upgrade.** Raw HTML export was converted to authored markdown (2026-07-15, commit `bb58039`); the SME then went beyond a faithful backfill, adding a 22-question quiz and per-module video scripts. Being treated as an active re-delivery upgrade (board: `Drafting`), not a pure fidelity import — track it on the board from here. Still needs a `00-design.md` (skipped) and an SME/refresh pass (Module 1 has dated claims: Windows 10 support "until Oct 2025", macOS Sonoma listed as current). | |
