...
| On-screen | Voiceover / talking points |
| --- | --- |
| Re-run the check on **Current Book** (Matthew). Show the now-short results panel with one result highlighted and the verse open alongside it. (`assets/ss-04-results-with-highlight.png`.) | "Now re-run the check — Current Book, Matthew — and look at the difference. The check finally has something to say, and it's a handful of results, every one of them meaningful. This is a list you can actually work." |
