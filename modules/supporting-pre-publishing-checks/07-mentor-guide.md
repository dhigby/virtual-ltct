...
**Purpose:** Guide for mentors assessing learner responses to the
[scenario bank](06-scenario-bank.md). This is the component that earns this course's
`3 - Independent` claim (see `00-design.md`, "Outcome-level open question") — score
reasoning and coaching quality, not a single correct fix. Several scenarios have more than
one defensible path.

## What "good" looks like — the scoring backbone

Pulled directly from the SME interview (`00-design.md`, "What 'good' looks like at the
target level"). Score every response against these eight markers, weighting the ones each
scenario is built to surface:

- **(a) Doesn't trust a clean result** — actively reveals denied or mass-approved items and
  re-checks (the spine/false-clean thread).
...
- **(g) Knows when to escalate** — an LT mentor for tooling problems, a Translation
  Consultant for content/translation decisions.
- **(h) Attention to detail**, especially in the formatting checks and the final
  read-through.

## What you're evaluating in every response

1. **Diagnosis** — did they identify the actual mechanism (not just restate the symptom),
   and did they say how they'd confirm it rather than assume it?
2. **What they'd say to the team** — is it a coaching conversation that leaves the fix in
   the team's hands, explained in plain terms, or does it slide into doing the work for
   them?
3. **Escalation judgment** — do they correctly sort "mine," "the team's," and "outside
   help" (an LT mentor for tooling, a Translation Consultant for content), and can they
   justify the sort rather than guessing?

## Providing feedback

**Strong responses:** acknowledge what they did well, name the specific "what good looks
like" marker(s) they hit, and suggest one refinement rather than a list.

**Adequate responses:** confirm what was correct, name the marker they missed, and ask a
probing question that would have surfaced it (e.g. "what would tell you the check was
actually re-run, rather than just showing an old result?").

**Weak responses:** identify where the reasoning went wrong against the backbone above,
work through the scenario together, and assign a re-read of the relevant lesson section
before reattempting.
...
produce the heading-doesn't-match-text flag and a garbled-looking
reference. Marker
...
Punctuation Inventory open on screen now, walked through live — not a description of what
...
(so the glossary link doesn't visually clutter every recurrence of a common term). This is
marker **(d)**: driving the tool correctly and being able to undo the bad move.

**Watch for in escalation:** the glossary's *content* is out of scope for the learner
(per the design doc's course scope) — if anything about the glossary content itself looks
wrong, that's not the LTC's to fix and isn't really an "escalation" so much as a scope
...
## Scenario 5: The Last Read-Through (Objective 9)

**Watch for in the diagnosis:** each of the three issues should get its own first-read
hypothesis and a way to confirm it — an orphan word usually points to a spacing/line-break
issue in the layout; a shifted footnote points to a page-flow change upstream; an awkward
heading at a column break is a layout/composition issue. The learner should not treat all
three as identical just because they surfaced in the same read-through.

**Watch for in what they'd say to the team:** the core of this scenario is the three-way
sort — "mine to resolve," "the team's call," "the typesetter's craft" — explained to the
team in terms they can follow, including *why* the LTC isn't trying to fix all three
personally. This directly tests marker **(f)**: knowing which surfaced issues are genuinely
someone else's to own.

**Watch for in escalation:** true typesetting composition (column balance, orphan/widow
control, the aesthetic call on where a heading lands at a break) defers to the typesetter,
per the design doc's objective 9 framing — the learner's job is to lead the read-through and
triage, not resolve typesetting craft themselves. A strong answer names specifically what
it would tell the typesetter so the issue isn't rediscovered from scratch (e.g. "footnote
for v.12 has drifted two pages from its reference — please check the page-break rule
there").

**Common wrong turn:** trying to personally resolve a genuine typesetting/composition
problem instead of triaging and handing it off with enough detail to be useful.

---

## Scenario 6: The Three Buckets (Objective 10)

**Watch for in the diagnosis:** this scenario is explicitly a test of the three-bucket
triage framework from Lesson 1 — the learner should sort the mis-set check status as
theirs (a tooling/process fix, marker **(a)**), the passage-reading disagreement as the
team's (a translation/content decision, marker **(f)**), and the markup problem beyond
their own diagnostic ability as an escalation (marker **(g)**). Look for the *reasoning*
behind each placement, not just the correct label — a learner who sorts all three
correctly but can't explain why hasn't demonstrated the judgment this course is assessing.

**Watch for in what they'd say to the team:** a short, plain explanation of why the learner
is handling one item directly, sending another back to the team, and bringing in outside
help for the third — without it reading as passing the buck. Strong answers frame this as
normal division of labor, not as the LTC being unable to help.

**Watch for in escalation:** for the markup problem beyond the learner's own diagnosis, the
correct destination is an LT mentor (tooling), not a Translation Consultant (content) —
watch for learners who default to "escalate to a Translation Consultant" for anything they
can't handle, rather than matching the *kind* of problem to the *kind* of escalation
contact. A strong answer also says what it would tell the LT mentor to get them up to speed
quickly (what's been tried, what the symptom looks like).

**Common wrong turn:** conflating the two escalation routes (LT mentor vs. Translation
Consultant), or trying to resolve the team's translation disagreement personally because
the team is asking the LTC to "just sort it out."

---

## Scenario 7 (combined): The Overloaded Setup (Objectives 3, 4, 8)
...
extra records — trimming it back to just what the project actually uses will speed Send/
Receive up again"). Credit answers that suggest a concrete right-sizing step (removing the
...
the design doc's note that this scenario confirms Digital and Print Publishing is honestly
earned by hands-on work, not just advice). A learner who escalates any of these three
outward has likely misjudged the scope of their own role at this level.
