...
- **Parsing & Concordance:** the parser suggests morpheme breakdowns based on existing lexicon entries; "Try a Word" lets you test the parser against a word without committing it to a text; the Word Analyses view lists every word in a text with its occurrence count and candidate analyses, and lets you approve/reassign analyses across the whole text (a concordance-style correction workflow). **Confirmed (SME, live 9.3.9):** the `Texts & Words` area actually has three distinct related views — **Word List Concordance** (a simple wordform + gloss list, confirmed and already used in Module 2 for reviewing interlinearized work), **Concordance**, and **Complex Concordance** — the latter two are still unverified and will need SME walkthrough before Module 3 describes what they do or how they differ from Word List Concordance.
