#!/usr/bin/env python3
"""ONE-TIME SEED importer for competency descriptors — retained for provenance.

This script originally generated `competencies/*.md` from two source documents (kept
alongside it in `import-seeds/`):
...
Those descriptors are now HAND-AUTHORED — `competencies/*.md` is the source of truth
(see CLAUDE.md). This importer is kept only to document how they were first seeded and
to re-seed a fresh checkout. Running it OVERWRITES every descriptor, discarding hand
edits, so it refuses to write unless given `--force`.

Run (re-seed, destructive):  python import-seeds/import_competency_descriptors.py --force
...
SEEDS = REPO / "import-seeds"
XLSX = SEEDS / "Lang Tech Competencies.xlsx"
CBC = SEEDS / "CBC Guide for Non-technical Competencies in Language Technology.md"
...
def main():
    if "--force" not in sys.argv[1:]:
        sys.exit(
            "Refusing to run: descriptors in competencies/ are now hand-authored "
            "(see CLAUDE.md). This one-time seed importer OVERWRITES them all, discarding "
            "hand edits. Re-run with --force only to re-seed from the import-seeds/ sources.")
    if not XLSX.exists():
    ...
    idx = ["# Competency Descriptors", "",
           f"_Seed snapshot ({today}) from `{XLSX.name}` (leveled) and `{CBC.name}` (Core). "
           "These files are now hand-authored — edit them directly; `competencies.yaml` "
           "remains the canonical name list and workflow state stays on the GitHub Project._",
           ""]
