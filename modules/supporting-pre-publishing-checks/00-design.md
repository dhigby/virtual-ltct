...
5. An expat project admin assumed **single-column** layout (the language has long words and the
   ...
   **Language of Wider Communication (LWC)** — this is a Bible translation project, so the
   ...
   the reader-expectation requirement with the team, then addressed hyphenation for the
   two-column layout. **(Mechanism corrected 2026-09-17, Jenni Beadle — Stage 5 fact-check,
   confirmed via a live screenshot):** hyphenation in Paratext is not hand-built — it is
   **automatically generated by the Wordlist tool**. The Wordlist's **View menu** has a
...
12. **(Added 2026-09-15, Jenni Beadle, Stage 5 fact-check correction.)** The structural-first
    formatting-check order taught in this course was missing its actual mandatory first step:
    **Chapter/Verse Numbers** — a distinct check category in Paratext's Run Basic Checks dialog,
    alongside Markers, Characters, Punctuation, References, and Footnote quotes. Every other check
    in this lesson (references, footnotes, verse-based markers, etc.) quotes/relies on chapter and
    verse locations to report its results — if there are chapter/verse numbering errors (e.g.
    duplicate verses), those other checks' reported locations become unreliable or ambiguous. So
    Chapter/Verse Numbers must always be confirmed clean **first**, before marker-pair census or
    anything else. The structural-first order is now 8 steps, not 7 (see the revised
    `04-formatting-and-references.md` module-breakdown row and objective 6).
...
- **Formatting & markup / Basic Checks**: wrong markers, unclosed footnote pairs (`\f…\f*`),
  ...
  Basic Checks has a genuine deny mechanism that spelling status does not); **skipping Chapter/
  Verse Numbers as the mandatory first check** (field case 12, **added 2026-09-15**, Jenni
  Beadle) — running marker-pair census or other checks before confirming chapter/verse numbers
  are clean makes those other checks' reported locations unreliable or ambiguous whenever a
  chapter/verse numbering error (e.g. a duplicate verse) is present.
